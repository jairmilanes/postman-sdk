...
describe("Postman SDK Integration Tests", async () => {
  ...
  it("should perform a post request", async () => {
    ...
    const results = await Collections.post(null, collectionData);
  ...
  it("should perform a delete request", async () => {
    ...
    const results = await Collections.destroy(collectionData.info._postman_id);

    expect(request.delete).toHaveBeenCalled();
    expect(request.delete).toHaveBeenCalledWith(
      getRequestData("DELETE", target, {}, collectionData.info._postman_id),
      expect.anything()
    );
